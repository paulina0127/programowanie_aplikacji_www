...
function addProduct() {
    ...
    $query = "INSERT INTO product_list (id, name, description, size, category, price, tax, quantity, available, photo, expires, created, modified) VALUES (NULL, '$name', '$description', '$size', '$category', '$price', '$tax', '$quantity', '$available', '$photo', '$expires', '$created', '$modified');";
    $result = mysqli_query($conn, $query);
...
function editProduct($id) {
    ...
    $query = "UPDATE product_list SET name='$name', description='$description', size='$size', category='$category', price='$price', tax='$tax', quantity='$quantity', available='$available', photo='$photo', expires='$expires', modified='$modified' WHERE id='$id_clear' LIMIT 1;";

    if (empty($_FILES['photo']['name'])) {
        $query = "UPDATE product_list SET name='$name', description='$description', size='$size', category='$category', price='$price', tax='$tax', quantity='$quantity', available='$available', expires='$expires', modified='$modified' WHERE id='$id_clear' LIMIT 1;";
    }
...
function productList()
{
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        $main_category = getCategory(getCategory($row['category'])['main_category'])['name'];
...
function editProductForm()
{
    ...
    $form .= '<form action="' . $_SERVER['REQUEST_URL'] . '" method="POST" enctype="multipart/form-data">
                ...
                <div class="double-form-field">
                    <div class="form-field">
                        <label for="size">Waga:</label>
                        <input type="text" name="size" id="size" value="' . getProduct($_GET['product'])['size'] . '" required />
                    </div>

                    <div class="form-field">
                        <label for="category">Kategoria:</label>
                        <input type="number" name="category" id="category" value="' . getProduct($_GET['product'])['category'] . '" required />
                    </div>
                </div>
